...
import google.registry.eppserver.EppProtocolModule.IpQuota;
...
public class EppServiceHandler extends SimpleChannelInboundHandler<ByteBuf> {
  ...
  private final QuotaManager connectionQuotaManager;
  private final QuotaManager commandQuotaManager;
  private final QuotaManager ipQuotaManager;
  ...
  private boolean ipQuotaAcquired = false;
  private boolean connectionQuotaAcquired = false;

  @Inject
  public EppServiceHandler(
      @Named("hello") byte[] helloBytes,
      FrontendMetrics metrics,
      @EppProtocol QuotaManager connectionQuotaManager,
      @CommandQuota QuotaManager commandQuotaManager,
      @IpQuota QuotaManager ipQuotaManager,
      @Named("idToken") Supplier<String> idTokenSupplier,
      @Config("projectId") String projectId) {
    this.helloBytes = helloBytes.clone();
    this.metrics = metrics;
    this.connectionQuotaManager = connectionQuotaManager;
    this.commandQuotaManager = commandQuotaManager;
    this.ipQuotaManager = ipQuotaManager;
  ...
  private void onSslHandshakeComplete(ChannelHandlerContext ctx, X509Certificate cert) {
    ...
    // We use the certificate hash for connection limits because clID isn't known yet.
    if (!ipQuotaManager.acquireQuota(new QuotaManager.QuotaRequest(clientAddress)).success()) {
      metrics.registerQuotaRejection("epp_connection_ip", clientAddress);
      Future<?> unusedFuture = ctx.close();
      return;
    }
    ipQuotaAcquired = true;

    if (!connectionQuotaManager
        .acquireQuota(new QuotaManager.QuotaRequest(sslClientCertificateHash))
        .success()) {
      metrics.registerQuotaRejection("epp_connection", sslClientCertificateHash);
      Future<?> unusedFuture = ctx.close();
      return;
    }
    connectionQuotaAcquired = true;
  ...
  private void handleEppFrame(ChannelHandlerContext ctx, ByteBuf frame) {
    if (ipQuotaAcquired) {
      ipQuotaManager.refreshQuota(new QuotaManager.QuotaRequest(clientAddress));
    }
    if (connectionQuotaAcquired) {
      connectionQuotaManager.refreshQuota(new QuotaManager.QuotaRequest(sslClientCertificateHash));
    }
    ...
    try {
      RegistryServlet.component.requestHandler().handleRequest(req, rsp);
  ...
  public void channelInactive(ChannelHandlerContext ctx) throws Exception {
    if (connectionQuotaAcquired) {
      connectionQuotaManager.releaseQuota(new QuotaManager.QuotaRebate(sslClientCertificateHash));
    }
    if (ipQuotaAcquired) {
      ipQuotaManager.releaseQuota(new QuotaManager.QuotaRebate(clientAddress));
    }
